...
/// $e$ are set to zero.
__INLINE gsl::floating_point frexp( real x )
{	
...
__INLINE real pow( real x, int n )
{	return gsl_pow_int(x,n);	}

/// These functions efficiently compute small integer powers
__INLINE real pow_2( real x ){	return gsl_pow_2( x );	}
__INLINE real pow_3( real x ){	return gsl_pow_3( x );	}
__INLINE real pow_4( real x ){	return gsl_pow_4( x );	}
__INLINE real pow_5( real x ){	return gsl_pow_5( x );	}
__INLINE real pow_6( real x ){	return gsl_pow_6( x );	}
__INLINE real pow_7( real x ){	return gsl_pow_7( x );	}
__INLINE real pow_8( real x ){	return gsl_pow_8( x );	}
__INLINE real pow_9( real x ){	return gsl_pow_9( x );	}

/// Determine the sign of x
__INLINE gsl::sign_type sign( real x )
{	return x >= 0 ? gsl::positive : gsl::negative;	}

/// Test x to see if it's odd
__INLINE bool is_odd( int x ){	return static_cast< bool >( x % 2 );	}

/// Test x to see if it's even
__INLINE bool is_even( int x ){	return ! is_odd(x);		}

/// return the min of two numbers, just wraps std::min
template< typename T > __INLINE T min( T x1, T x2 ){	return std::min( x1, x2 );	}

/// return the max of two numbers, just wraps std::max
template< typename T > __INLINE T max( T x1, T x2 ){	return std::max( x1, x2 );	}
...
__INLINE gsl::equality_type fcmp( real x, real y, real epsilon)
